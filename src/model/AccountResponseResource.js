...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['ApiClient', 'model/AccountClass', 'model/AccountLinks', 'model/AccountTransactionInterval'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    module.exports = factory(require('../ApiClient'), require('./AccountClass'), require('./AccountLinks'), require('./AccountTransactionInterval'));
  } else {
    // Browser globals (root is window)
    if (!root.BasiqApi) {
      root.BasiqApi = {};
    }
    root.BasiqApi.AccountResponseResource = factory(root.BasiqApi.ApiClient, root.BasiqApi.AccountClass, root.BasiqApi.AccountLinks, root.BasiqApi.AccountTransactionInterval);
  }
}(this, function(ApiClient, AccountClass, AccountLinks, AccountTransactionInterval) {
  'use strict';

  /**
   * The AccountResponseResource model module.
   * @module model/AccountResponseResource
   * @version 2.0.0
   */

  /**
   * Constructs a new <code>AccountResponseResource</code>.
   * Container object, containing account details.  The account object represents an account held with a financial institution (e.g. a savings account). You can use this object to retrieve specific account details such as the account number, balance and available funds.
   * @alias module:model/AccountResponseResource
   * @class
   * @param accountHolder {String} The name of the account holder as returned by the institution. No formatting is applied. Returns a string or null when not available.
   * @param accountNo {String} Full account number.
   * @param availableFunds {String} Funds that are available to an account holder for withdrawal or other use. This may include funds from an overdraft facility or line of credit. As well as funds classified as the available balance, such as from cleared and existing deposits.
   * @param balance {String} Amount of funds in the account right now - excluding any pending transactions. For a credit card this would be zero or the minus amount spent.
   * @param _class {module:model/AccountClass} 
   * @param connection {String} The id of the connection resource that was used to retrieve the account.
   * @param currency {String} The currency the funds are stored in, using ISO 4217 standard.
   * @param id {String} Uniquely identifies the account.
   * @param institution {String} The id of the institution resource the account originated from.
   * @param lastUpdated {String} Timestamp of last update, UTC, RFC 3339 format e.g. \"2017-09-28T13:39:33Z\"
   * @param links {module:model/AccountLinks} 
   * @param name {String} Account name as defined by institution or user.
   * @param status {module:model/AccountResponseResource.StatusEnum} Indicates the account status. Always set to 'available'. Field kept for backward compatibility. Possible values include: <ul><li>available newest account data is available.</li></ul>
   * @param transactionIntervals {Array.<module:model/AccountTransactionInterval>} An array of date intervals indicating the coverage of the transaction data relating to the account. Will return a single element for accounts sourced from a single bank connection. Will return multiple elements in cases where there have been multiple PDF/CSV uploads for an account.
   * @param type {String} Always \"account\".
   */
  var exports = function(accountHolder, accountNo, availableFunds, balance, _class, connection, currency, id, institution, lastUpdated, links, name, status, transactionIntervals, type) {
    ...
    this.name = name;
    this.status = status;
    this.transactionIntervals = transactionIntervals;
    this.type = type;
  };

  /**
   * Constructs a <code>AccountResponseResource</code> from a plain JavaScript object, optionally creating a new instance.
   * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
   * @param {Object} data The plain JavaScript object bearing properties of interest.
   * @param {module:model/AccountResponseResource} obj Optional instance to populate.
   * @return {module:model/AccountResponseResource} The populated <code>AccountResponseResource</code> instance.
   */
  exports.constructFromObject = function(data, obj) {
    if (data) {
      ...
      if (data.hasOwnProperty('availableFunds'))
        obj.availableFunds = ApiClient.convertToType(data['availableFunds'], 'String');
      if (data.hasOwnProperty('balance'))
        obj.balance = ApiClient.convertToType(data['balance'], 'String');
      if (data.hasOwnProperty('class'))
        obj._class = AccountClass.constructFromObject(data['class']);
      ...
      if (data.hasOwnProperty('transactionIntervals'))
        obj.transactionIntervals = ApiClient.convertToType(data['transactionIntervals'], [AccountTransactionInterval]);
  ...
  /**
   * @member {module:model/AccountClass} _class
   */
  ...
  /**
   * Indicates the account status. Always set to 'available'. Field kept for backward compatibility. Possible values include: <ul><li>available newest account data is available.</li></ul>
   * @member {module:model/AccountResponseResource.StatusEnum} status
   */
   ...
   * @member {Array.<module:model/AccountTransactionInterval>} transactionIntervals
   */
  exports.prototype.transactionIntervals = undefined;
