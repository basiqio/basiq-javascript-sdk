...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['ApiClient', 'model/SourceName'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    module.exports = factory(require('../ApiClient'), require('./SourceName'));
  } else {
    // Browser globals (root is window)
    if (!root.BasiqApi) {
      root.BasiqApi = {};
    }
    root.BasiqApi.AccountsFeatures = factory(root.BasiqApi.ApiClient, root.BasiqApi.SourceName);
  }
}(this, function(ApiClient, SourceName) {
  'use strict';

  /**
   * The AccountsFeatures model module.
   * @module model/AccountsFeatures
   * @version 2.0.0
   */

  /**
   * Constructs a new <code>AccountsFeatures</code>.
   * AccountsFeatures describes set of institution accounts features
   * @alias module:model/AccountsFeatures
   * @class
   * @param accountHolder {Array.<module:model/SourceName>} AccountHolder holds list of data source identifiers which are capable to fetch account holder.
   * @param accountNo {Array.<module:model/SourceName>} AccountNumber holds list of data source identifiers which are capable to fetch account number.
   * @param availableFunds {Array.<module:model/SourceName>} AvailableFunds holds list of data source identifiers which are capable to fetch available funds.
   * @param balance {Array.<module:model/SourceName>} AccountBalance holds list of data source identifiers which are capable to fetch account balance.
   * @param currency {Array.<module:model/SourceName>} AccountCurrency holds list of data source identifiers which are capable to fetch account currency.
   * @param lastUpdated {Array.<module:model/SourceName>} LastUpdated holds list of data source identifiers which are capable to fetch account last updated date.
   * @param meta {Array.<module:model/SourceName>} Meta holds list of data source identifiers which are capable to fetch account meta data (e.g. mortgage data).
   * @param name {Array.<module:model/SourceName>} AccountName holds list of data source identifiers which are capable to fetch account name.
   */
  var exports = function(accountHolder, accountNo, availableFunds, balance, currency, lastUpdated, meta, name) {
  ...
  /**
   * Constructs a <code>AccountsFeatures</code> from a plain JavaScript object, optionally creating a new instance.
   * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
   * @param {Object} data The plain JavaScript object bearing properties of interest.
   * @param {module:model/AccountsFeatures} obj Optional instance to populate.
   * @return {module:model/AccountsFeatures} The populated <code>AccountsFeatures</code> instance.
   */
  exports.constructFromObject = function(data, obj) {
    if (data) {
      obj = obj || new exports();
      if (data.hasOwnProperty('accountHolder'))
        obj.accountHolder = ApiClient.convertToType(data['accountHolder'], [SourceName]);
      if (data.hasOwnProperty('accountNo'))
        obj.accountNo = ApiClient.convertToType(data['accountNo'], [SourceName]);
      if (data.hasOwnProperty('availableFunds'))
        obj.availableFunds = ApiClient.convertToType(data['availableFunds'], [SourceName]);
      if (data.hasOwnProperty('balance'))
        obj.balance = ApiClient.convertToType(data['balance'], [SourceName]);
      if (data.hasOwnProperty('currency'))
        obj.currency = ApiClient.convertToType(data['currency'], [SourceName]);
      if (data.hasOwnProperty('lastUpdated'))
        obj.lastUpdated = ApiClient.convertToType(data['lastUpdated'], [SourceName]);
      if (data.hasOwnProperty('meta'))
        obj.meta = ApiClient.convertToType(data['meta'], [SourceName]);
      if (data.hasOwnProperty('name'))
        obj.name = ApiClient.convertToType(data['name'], [SourceName]);
    }
    return obj;
  }
   ...
   * @member {Array.<module:model/SourceName>} accountHolder
   */
   ...
   * @member {Array.<module:model/SourceName>} accountNo
   */
   ...
   * @member {Array.<module:model/SourceName>} availableFunds
   */
